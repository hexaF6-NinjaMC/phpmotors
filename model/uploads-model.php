<?php
    ...
    function checkExistingImage($imgName, $invId) {
        $db = phpmotorsConnect();
        $sql = "SELECT imgName FROM images WHERE imgName = :name AND images.invId = :invId";
        $stmt = $db->prepare($sql);
    ...
    function getThumbnailsPathById($vehicleId) {
        $db = phpmotorsConnect();
        $sql = "SELECT imgPath FROM images WHERE invId = :invId AND imgPath LIKE '%-tn.%'";
        $stmt = $db->prepare($sql);
        $stmt->bindValue(':invId', $vehicleId, PDO::PARAM_STR);
        $stmt->execute();
        $imageArr = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $stmt->closeCursor();
        return $imageArr;
    }

    function getPrimaryImgPathById($invId) {
        $db = phpmotorsConnect();
        $sql = "SELECT imgPath FROM images WHERE invId = :invId AND imgPath NOT LIKE '%-tn.%' AND imgPrimary = 1";
        $stmt = $db->prepare($sql);
